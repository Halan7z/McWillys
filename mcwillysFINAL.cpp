...
vector<ClienteAfiliado> clientesAfiliados = {};
...
public:
    ...
    void mostrarMenuMcWillysCLIENTE(informacionCajero& info);
    ...
    void generarFacturaCLIENTE(const Orden& orden, informacionCajero& info, bool);
    void generarFacturaCLIENTEAFILIADO(const Orden& orden, informacionCajero& info, bool, int);
...
double sumarValoresArchivo(const string& nombreArchivo);
void imprimirBalanceGeneral();
...
void afiliarseRestaurante();
bool verificarAfiliacionCliente(Orden& orden);
void aplicarBeneficios(const Orden& orden);
...
void menu::menuPrincipal()
{
    ...
    string contrasenaGerente = "ghXk1799";
    ...
    do {
        ...
        cout << "3. Opción para Comprador" << endl;
        cout << "4. Salir" << endl;
        ...
        switch (opcion) {
            ...
            case 3: {
                system("cls");
                cout << "Has seleccionado la opción para Comprador.\n" << endl;

                informacionCajero info; // Declarar info fuera del bucle
                vector<ClienteAfiliado> clientesAfiliados;
                // Submenú para Comprador
                while (true) {
                    int opcionComprador;
                    system("cls");
                    cout << "Menu para Clientes:" << endl;
                    cout << "1. Comprar" << endl;
                    cout << "2. Afiliarse a McWillys" << endl;
                    cout << "3. Salir" << endl;
                    cout << "Ingresa la opción: ";
                    cin >> opcionComprador;
                    ...
                    switch (opcionComprador) {
                        case 1:
                            mostrarMenuMcWillysCLIENTE(info);
                            break;
                        case 2:
                            system("cls");
                            afiliarseRestaurante(); // Pasar clientesAfiliados por referencia
                            break;
                        case 3:
                            system("cls");
                            // Opción Salir del submenu
                            break;
                        default:
                            cout << "Opción no válida. Por favor ingresa una opción válida." << endl;
                            break;
...
void aumentarStock(const char *archivo, const string &ingrediente, int cantidad, int seleccion) {
    ...
    ofstream archivoGastos("gastos.txt", ios::app); // Abre el archivo en modo de agregar al final
    if (archivoGastos.is_open()) {
        archivoGastos << total << endl; // Guarda el total a pagar y agrega un salto de línea
        archivoGastos.close();
    } else {
...
void mostrarInventario(const char *archivo) {
    vector<string> proveedores = {"Sofia", "Granjas Pabon", "Hipermaxi", "Granjas Pabon",
        "Hipermaxi", "Ketal", "Ketal", "San Gabriel", "San Gabriel", "Gustossi", "Granjas Pabon",
        "Hipermaxi", "Hipermaxi", "Kriss", "Kriss", "Kriss", "McWillys", "McWillys", "McWillys",
        "McWillys", "McWillys", "McWillys", "Hipermaxi", "Granjas Pabon", "Ketal", "Hipermaxi",
        "Hipermaxi", "Hipermaxi", "Coca Cola", "Nestle", "Delizia", "Hipermaxi", "Hipermaxi", "Ketal",
        "McWillys"
    };
    ...
    cout << "+--------------------------+---------------------+---------------------+" << endl;
    cout << "| " << left << setw(25) << "Producto" << "| " << setw(10) << right << "Unidades            |"
         << " " << setw(20) << "Proveedor" << "|" << endl;
    cout << "+--------------------------+---------------------+---------------------+" << endl;

    size_t index = 0;  // Índice para recorrer el vector de proveedores
    ...
    while (getline(readFile, line)) {
        ...
        // Asegurarse de no exceder el tamaño del vector de proveedores
        index = index % proveedores.size();

        cout << "| " << left << setw(25) << producto << "| " << setw(10) << right << cantidad << " unidades |"
             << " " << setw(20) << proveedores[index] << "|" << endl;
...
void imprimirBalanceGeneral() {
    // Obtener las sumas de ganancias y gastos
    double ganancias = sumarValoresArchivo("ganancias.txt");
    double gastos = sumarValoresArchivo("gastos.txt");

    // Calcular las utilidades (ganancias - gastos)
    double utilidades = ganancias - gastos;

    // Imprimir el balance general
    cout << "======= BALANCE GENERAL =======\n";
    cout << "Ganancias: " << ganancias << " Bs\n";
    cout << "Gastos: " << gastos << " Bs\n";
    cout << "Utilidades: " << utilidades << " Bs\n";
    cout << "===============================" << endl;
    cout << endl;
    system("pause");
    system("cls");
            ...
            case 2:
                ...
                switch(seleccion)
                {
                    case 1:
                        ...
                        restarStock(archivo, "Salsa especial", 1*cantidad);
                        restarStock(archivo, "Tortilla", 1*cantidad);
                        break;
                    case 2:
                        restarStock(archivo, "Tocino", 5*cantidad);
                        ...
                        break;
                    case 3:
                        restarStock(archivo, "Carne de res", 2*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        restarStock(archivo, "Vegetales", 1*cantidad);
                        restarStock(archivo, "Queso", 2*cantidad);
                        restarStock(archivo, "Salsa especial", 1*cantidad);
                        restarStock(archivo, "Mayonesa", 1*cantidad);
                        restarStock(archivo, "Ketchup", 1*cantidad);
                        restarStock(archivo, "Pepinillos", 1*cantidad);
                        break;
                    case 4:
                        ...
                        restarStock(archivo, "Queso", 1*cantidad);
                        restarStock(archivo, "Salsa especial", 1*cantidad);
                        restarStock(archivo, "Salsa programacion", 1*cantidad);
                        restarStock(archivo, "Pollo", 1*cantidad);
                        break;
                    case 5:
                        restarStock(archivo, "Carne de res", 1*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        restarStock(archivo, "Vegetales", 1*cantidad);
                        restarStock(archivo, "Queso", 1*cantidad);
                        restarStock(archivo, "Salsa especial", 2*cantidad);
                        restarStock(archivo, "Papas", 1*cantidad);
                        break;
                    case 6:
                        restarStock(archivo, "Carne especial", 2*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        restarStock(archivo, "Vegetales", 1*cantidad);
                        restarStock(archivo, "Queso suizo", 2*cantidad);
                        restarStock(archivo, "Salsa programacion", 2*cantidad);
                        break;
                    case 7:
                        restarStock(archivo, "Pollo", 1*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        ...
                        restarStock(archivo, "Queso", 1*cantidad);
                        restarStock(archivo, "Salsa programacion", 2*cantidad);
                        restarStock(archivo, "Salsa tenorio", 1*cantidad);
                        restarStock(archivo, "Tocino", 4*cantidad);
                        break;
                    case 8:
                        restarStock(archivo, "Carne vegetariana", 1*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        restarStock(archivo, "Vegetales", 2*cantidad);
                        restarStock(archivo, "Queso", 1*cantidad);
                        restarStock(archivo, "Salsa vino tinto", 1*cantidad);
                        break;
                    case 9:
                        restarStock(archivo, "Carne de res", 2*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        restarStock(archivo, "Queso", 1*cantidad);
                        restarStock(archivo, "Salsa BBQ", 1*cantidad);
                        restarStock(archivo, "Salsa picante", 2*cantidad);
                        restarStock(archivo, "Jalapeños", 5*cantidad);
                        restarStock(archivo, "Pollo", 1*cantidad);
                        break;
                    case 10:
                        restarStock(archivo, "Carne de res", 2*cantidad);
                        restarStock(archivo, "Pollo", 1*cantidad);
                        restarStock(archivo, "Tocino", 3*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        restarStock(archivo, "Queso", 2*cantidad);
                        restarStock(archivo, "Salsa programacion", 2*cantidad);
                        restarStock(archivo, "Pepinillos", 5*cantidad);
                        restarStock(archivo, "Champiñones", 5*cantidad);
                        break;
                    case 11:
                        restarStock(archivo, "Carne especial", 1*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        restarStock(archivo, "Queso", 5*cantidad);
                        restarStock(archivo, "Queso suizo", 1*cantidad);
                        restarStock(archivo, "Salsa tenorio", 5*cantidad);
                        restarStock(archivo, "Salsa programacion", 1*cantidad);
                        restarStock(archivo, "Tocino", 10*cantidad);
                        break;
                    case 12:
                        restarStock(archivo, "Carne especial", 2*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                    ...
                    case 13:
                        ...
                        restarStock(archivo, "Mayonesa", 1*cantidad);
                        restarStock(archivo, "Ketchup", 1*cantidad);
                        restarStock(archivo, "Mostaza", 1*cantidad);
                        restarStock(archivo, "Palza pie", 1*cantidad);
                        break;
                    case 14:
                        restarStock(archivo, "Carne silpancho", 2*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        restarStock(archivo, "Queso", 1*cantidad);
                        restarStock(archivo, "Salsa tenrio", 1*cantidad);
                        restarStock(archivo, "Salsa picante", 1*cantidad);
                        restarStock(archivo, "Jalapeños", 1*cantidad);
                        break;
                    case 15:
                        restarStock(archivo, "Carne de res", 2*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        restarStock(archivo, "Queso", 1*cantidad);
                        restarStock(archivo, "Vegetales", 2*cantidad);
                        restarStock(archivo, "Mayonesa", 3*cantidad);
                        restarStock(archivo, "Ketchup", 3*cantidad);
                        restarStock(archivo, "Mostaza", 3*cantidad);
                        restarStock(archivo, "Pepinillos", 4*cantidad);
                        break;
                    case 16:
                        restarStock(archivo, "Carne especial", 2*cantidad);
                        restarStock(archivo, "Pan", 1*cantidad);
                        restarStock(archivo, "Queso gouda", 5*cantidad);
                        restarStock(archivo, "Vegetales", 2*cantidad);
                        restarStock(archivo, "Salsa de vino tinto", 2*cantidad);
                        restarStock(archivo, "Tocino", 5*cantidad);
                        restarStock(archivo, "Champiñones", 3*cantidad);
                        restarStock(archivo, "Salsa especial", 2*cantidad);
                        break;
                }
                break;
        ...
        switch (opcion) {
            ...
            case 2:
                ...
                switch(seleccion)
                {
                    case 1:
                    {
                       restarStock(archivo, "Palza cakes", 1*cantidad);
                    }
                    case 2:
                    {
                       restarStock(archivo, "Wafles", 5*cantidad);
                       restarStock(archivo, "Helado", 1*cantidad);
                    }
                    case 3:
                    {
                       restarStock(archivo, "Pan hot dog", 1*cantidad);
                       restarStock(archivo, "Salchicha", 1*cantidad);
                       restarStock(archivo, "Mayonesa", 1*cantidad);
                       restarStock(archivo, "Ketchup", 1*cantidad);
                       restarStock(archivo, "Mostaza", 1*cantidad);
        ...
        switch (opcion) {
            ...
            case 2:
                ...
                switch(seleccion)
                {
                    case 1:
                    {
                       restarStock(archivo, "Pollo", 3*cantidad);
                       restarStock(archivo, "Salsa programacion", 3*cantidad);
                       restarStock(archivo, "Mayonesa", 1*cantidad);
                       restarStock(archivo, "Ketchup", 1*cantidad);
                       restarStock(archivo, "Mostaza", 1*cantidad);
                    }
                    case 2:
                    {
                       restarStock(archivo, "Soda", 1*cantidad);
                    }
                    case 3:
                    {
                       restarStock(archivo, "Cafe", 1*cantidad);
                    }
                    case 4:
                    {
                       restarStock(archivo, "Helado", 3*cantidad);
                    }
                    case 5:
                    {
                       restarStock(archivo, "Papas", 2*cantidad);
                       restarStock(archivo, "Salsa programacion", 1*cantidad);
                       restarStock(archivo, "Mayonesa", 1*cantidad);
                       restarStock(archivo, "Ketchup", 1*cantidad);
                       restarStock(archivo, "Mostaza", 1*cantidad);
        ...
        switch (opcion) {
            ...
            case 2:
                ...
                switch(seleccion)
                {
                    case 1:
                    {
                       restarStock(archivo, "Galletas", 5*cantidad);
                    }
                    case 2:
                    {
                       restarStock(archivo, "Sandwich de helado", 3*cantidad);
                    }
                    case 3:
                    {
                       restarStock(archivo, "Helado", 5*cantidad);
                    }
                    case 4:
                    {
                       restarStock(archivo, "Cupcakes", 3*cantidad);
                    }
                    case 5:
                    {
                       restarStock(archivo, "Helado", 1*cantidad);
                        ...
                        if (cantidad > 0) {
                            ...
                            /* switch(opcionOferta)
                            {
                                case 1:
                                {
                                    ...
                                    restarStock(archivo, "Carne especial", 1*2*cantidad);
                                    restarStock(archivo, "Pan", 1*2*cantidad);
                                    restarStock(archivo, "Queso", 5*2*cantidad);
                                    restarStock(archivo, "Queso suizo", 1*2*cantidad);
                                    restarStock(archivo, "Salsa tenorio", 5*2*cantidad);
                                    restarStock(archivo, "Salsa programacion", 1*2*cantidad);
                                    restarStock(archivo, "Tocino", 10*2*cantidad);
                                    restarStock(archivo, "Soda", 1*cantidad);
                                }
                                    break;
                                ...
                                case 4:
                                {
                                   restarStock(archivo, "Helado", 10*cantidad);
                                }
                                    break;
                                case 5:
                                {
...
void menu::generarFactura(const Orden& orden, informacionCajero& info) {
    ...
    int numeroActualFactura;
    ifstream archivoEntrada("contador_facturas.txt");
    if (archivoEntrada.is_open()) {
        archivoEntrada >> numeroActualFactura;
        archivoEntrada.close();
    } else {
        // inicializador del contador
        numeroActualFactura = 0;
    }

    // incrementar contador
    ++numeroActualFactura;
    ofstream archivoSalida("contador_facturas.txt");
    if (archivoSalida.is_open()) {
        archivoSalida << numeroActualFactura;
        archivoSalida.close();
    }
    ...
    cout << "----------------------------------------------------------------------" << endl;
    cout << "Factura Nº: " << numeroActualFactura << endl;
    cout << "Cajero: " << info.nombre << " (Código: " << info.numerocarnet << ")" << endl;
    ...
    cout << "----------------------------------------------------------------------" << endl;
    cout << setw(30) << left << "| Producto" << setw(23) << right << "| Precio (Bs) |" << endl;
    cout << "----------------------------------------------------------------------" << endl;

    for (size_t i = 0; i < orden.productos.size(); ++i) {
        cout << "| " << setw(29) << left << orden.productos[i]  << "| " << setw(15) << right << orden.preciosEnConjunto[i] << " Bs |" << endl;
    }

    cout << "----------------------------------------------------------------------" << endl;
    cout << "| Total a pagar: " << setw(31) << right << orden.total << " Bs |" << endl;
    cout << "======================================================================" << endl;
    cout << "\n¡Gracias por su compra!\n" << endl;
    cout << "Si requiere factura, solicítela en este restaurante o para su mayor \ncomodidad y rapidez obténgala en www.facturacionmcwillys.com.bo" << endl;
    cout << "======================================================================" << endl;
    ...
    if (ss.fail()) {
        cerr << "Error al obtener la fecha de la orden." << endl;
        return;
    }
    ...
    if (stat(rutaCarpeta.c_str(), &fileStat) != 0) {
    ...
    string nombreArchivo = rutaCarpeta + "/factura_";
    ...
    if (archivo.is_open()) {
        archivo << "======================================================================" << endl;
        archivo << "\t\t\tFACTURA GENERADA\t\t\t\t" << endl;
        archivo << "======================================================================" << endl;
        archivo << "Restaurantes Ciudad de La Paz" << endl;
        archivo << "Green tower, Edificio Horizonte, Av. Ballivián 1063, La Paz" << endl;
        archivo << "----------------------------------------------------------------------" << endl;
        archivo << "Factura Nº: " << numeroArchivo << endl;
        archivo << "Cajero: " << "Nombre del cajero" << " (Código: " << "Código del cajero" << ")" << endl;

        time_t now = time(0);
        tm* tiempoActual = localtime(&now);
        archivo << "Fecha de venta: " << orden.fechaVenta << endl;


        archivo << "----------------------------------------------------------------------" << endl;
        archivo << "Cliente: " << orden.nombreCliente << endl;
        archivo << "Carnet o NIT: " << orden.carnetONit << endl;
        archivo << "----------------------------------------------------------------------" << endl;
        archivo << setw(30) << left << "| Producto" << setw(23) << right << "| Precio (Bs) |" << endl;
        archivo << "----------------------------------------------------------------------" << endl;

        for (size_t i = 0; i < orden.productos.size(); ++i) {
            archivo << "| " << setw(29) << left << orden.productos[i] << "| " << setw(15) << right << orden.preciosEnConjunto[i] << " Bs |" << endl;
        }

        archivo << "----------------------------------------------------------------------" << endl;
        archivo << "| Total a pagar: " << setw(31) << right << orden.total << " Bs |" << endl;
        archivo << "======================================================================" << endl;
        archivo << "\n¡Gracias por su compra!\n" << endl;
        archivo << "Si requiere factura, solicítela en este restaurante o para su mayor \ncomodidad y rapidez obténgala en www.facturacionmcwillys.com.bo" << endl;
        archivo << "======================================================================" << endl;

        archivo.close();
    } else {
        cerr << "Error al abrir el archivo de órdenes individuales." << endl;
        return;


    }

    ofstream archivoGanancias("ganancias.txt", ios::app); // Abre el archivo en modo de agregar al final
    if (archivoGanancias.is_open()) {
        archivoGanancias << orden.total << endl; // Guarda el total a pagar y agrega un salto de línea
        archivoGanancias.close();
    } else {
        cout << "Error al abrir el archivo de ganancias." << endl;
    }
}
...
// --------------------------------------------------
// Función para afiliarse al restaurante

void afiliarseRestaurante()
{
    ClienteAfiliado nuevoCliente;

    cout << "¡Bienvenido al programa de afiliación!" << endl;
    cout << "Ingrese su nombre completo: ";
    cin.ignore();
    getline(cin, nuevoCliente.nombreCompleto);
    cout << "Ingrese su carnet de identidad (como código de cliente): ";
    cin >> nuevoCliente.carnetIdentidad;

    // Verificar si el cliente ya está afiliado
    for (const auto& cliente : clientesAfiliados) {
        if (cliente.carnetIdentidad == nuevoCliente.carnetIdentidad) {
            cout << "¡Usted ya está afiliado! No es necesario asociarse nuevamente." << endl;
            system("pause");
            return;
        }
    }

    // Agregar al nuevo cliente a la lista de afiliados
    clientesAfiliados.push_back(nuevoCliente);
    cout << "¡Felicidades! Se ha afiliado exitosamente." << endl;
    system("pause");

    // Guardar la información en un archivo de texto
    ofstream archivoAfiliados("clientes_afiliados.txt", ios::app); // ios::app para agregar al final del archivo

    if (archivoAfiliados.is_open()) {
        archivoAfiliados << nuevoCliente.nombreCompleto << " " << nuevoCliente.carnetIdentidad << endl;
        archivoAfiliados.close();
        cout << "Información guardada en el archivo clientes_afiliados.txt" << endl;
    } else {
...
// --------------------------------------------------
void menu::mostrarMenuMcWillysCLIENTE(informacionCajero& info) {
    int opcion;
    Orden orden;
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 6:
                system("cls");
                cout << "Terminando de procesar y pidiendo su orden..." << endl;
                cout << "Ingrese su nombre: ";
                cin.ignore();
                getline(cin, orden.nombreCliente);
                cout << "Ingrese su carnet o NIT: ";
                getline(cin, orden.carnetONit);
                cout << "Ingrese la fecha: ";
                getline(cin, orden.fechaVenta);

                // Verificar afiliación del cliente
                bool esAfiliado = verificarAfiliacionCliente(orden);

                // Aplicar beneficios si es afiliado

                int total_aux = orden.total;

                if (esAfiliado)
                {
                    total_aux*= 0.9;
                    generarFacturaCLIENTEAFILIADO(orden, info, esAfiliado, total_aux);
                }
                else
                {
                    generarFacturaCLIENTE(orden, info, esAfiliado);
                }

                // Resto del código para guardar la orden
                guardarOrdenMes(orden);
                guardarOrden(orden);
                break;
        }
...
void menu::generarFacturaCLIENTE(const Orden& orden, informacionCajero& info, bool esAfiliado) {
    system("cls");
    ...
    cout << "Green tower, Edificio Horizonte, Av. Ballivián 1063, La Paz" << endl;
    cout << "----------------------------------------------------------------------" << endl;
    cout << "Cajero: ----"<< " (Código: ----)" << endl;
    cout << "Factura Nº: " << numeroActualFactura << endl;

    time_t now = time(0);
    tm* tiempoActual = localtime(&now);
    cout << "Fecha de venta: " << tiempoActual->tm_mday << "/"
         << (tiempoActual->tm_mon + 1) << "/"
         << (tiempoActual->tm_year + 1900) << endl;
    ...
    for (size_t i = 0; i < orden.productos.size(); ++i)
        cout << "|" << setw(29) << left << orden.productos[i] << "| " << setw(16) << right << orden.preciosEnConjunto[i] << " Bs |" << endl;
    cout << "----------------------------------------------------------------------" << endl;
    cout << "| Total a pagar: " << setw(31) << right << orden.total << " Bs |" << endl;
    cout << "======================================================================" << endl;
    cout << "\n¡Gracias por su compra!\n" << endl;
    cout << "Si requiere factura, solicítela en este restaurante o para su mayor \ncomodidad y rapidez obténgala en www.facturacionmcwills.com.bo" << endl;
    cout << "======================================================================" << endl;
...
void menu::generarFacturaCLIENTEAFILIADO(const Orden& orden, informacionCajero& info, bool esAfiliado, int total_aux) {
    system("cls");
    ...
    for (size_t i = 0; i < orden.productos.size(); ++i)
        cout << "|" << setw(29) << left << orden.productos[i] << "| " << setw(16) << right << orden.preciosEnConjunto[i] << " Bs |" << endl;
    cout << "----------------------------------------------------------------------" << endl;
    cout << "| Total a pagar: " << setw(31) << right << total_aux << " Bs |" << endl;
    cout << "======================================================================" << endl;

    if (esAfiliado)
    {
        cout << "Cliente afiliado" << endl;
        aplicarBeneficios(orden);
    };
    ...
    ofstream archivoGanancias("ganancias.txt", ios::app); // Abre el archivo en modo de agregar al final
    if (archivoGanancias.is_open()) {
        archivoGanancias << total_aux << endl; // Guarda el total a pagar y agrega un salto de línea
        archivoGanancias.close();
    } else {
        cout << "Error al abrir el archivo de ganancias." << endl;
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Función para verificar si un cliente está afiliado

bool verificarAfiliacionCliente(Orden& orden) {
    ifstream archivo("clientes_afiliados.txt");
    if (!archivo.is_open()) {
        cerr << "Error al abrir el archivo de clientes afiliados." << endl;
        return false;
    }

    // Variables temporales para leer desde el archivo
    string nombreCompleto;
    string carnetIdentidad;

    // Leer datos del archivo y comparar con la orden.carnet
    while (archivo >> nombreCompleto >> carnetIdentidad) {
        if (carnetIdentidad == orden.carnetONit) {
            archivo.close(); // Cerrar el archivo después de encontrar una coincidencia
            return true;     // Cliente afiliado encontrado
        }
    }

    archivo.close(); // Cerrar el archivo si no se encontró ninguna coincidencia
    return false;    // Cliente no afiliado
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
void aplicarBeneficios(const Orden& orden) {

    // Aplicar descuento para clientes afiliados

    // Agregar detalles adicionales para clientes afiliados
    // Puedes personalizar esta sección según tus necesidades
    cout << "| Descuento (10%): " << setw(28) << right << orden.total * 0.1 << " Bs |" << endl;
    cout << "| Beneficio: Descuento del 10% |" << endl;
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
